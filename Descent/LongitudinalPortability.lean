...
`Descent.LDDecayTheory`, the one per-generation drift rate, and this file
imports it.
...
`Descent.LDDecayTheory`, which this module imports.  It was stated again here as
...
rate off it. `Descent.HorizonCurve` supplies three facts that decide when that reading is
legitimate.
...
`Descent.DriftingConditional` settles when they can be told apart. At any fixed cohort they
