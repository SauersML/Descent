...
`Descent.BundleRigidity`, pointing the other way: rigidity holds for realized panels
...
answers with a bit. `Descent.JetBarrier` already shows that bit is readable by a
