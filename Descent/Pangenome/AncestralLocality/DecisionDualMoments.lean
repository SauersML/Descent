...
(`hasDerivWithinAt_duhamel`). For a family that starts at zero and satisfies `|m_t(f)| ≤ K ‖f‖`, a
mean value bound along this curve and induction on the number `j` of branchings give
`|m_t(f)| ≤ K ‖f‖ C(j + n, n) (R t)^j` for every `j` (`abs_moment_le_choose_mul_pow_right`). These
are the terms of a convergent series when `R t < 1`, so `m_t = 0` there
(`moment_eq_zero_of_mul_lt_one_right`), and shifting time extends this to every `t ≥ 0`
(`moment_eq_zero_right`). Two bounded families obeying the moment equation with the same initial
moments agree at every time (`moments_eq_of_rightMomentEquation`).

The argument reads the family only at times `s ≥ 0`: the bound, the moment equation as a right
derivative, and continuity of every moment on `[0, ∞)` (`continuousOn_duhamel`). A dual
expectation constructed for `t ≥ 0` has exactly these properties. Families with a two-sided
derivative at every real time are the special case (`hasDerivAt_duhamel`,
`abs_moment_le_choose_mul_pow`, `moment_eq_zero_of_mul_lt_one`, `moment_eq_zero`,
`moments_eq_of_momentEquation`).
...
vectors), and the moment equation is assumed at every time `s ≥ 0` as a right derivative, together
with continuity of the moments on `[0, ∞)`.
...
/-- **Duhamel's formula along the coalescence gain, within a set.** If every moment `u ↦ m_u(g)`
has derivative `momentGenerator c r T m_s g` at `s` within `S`, then
`u ↦ e^{-λ(t-u)} m_u(e^{(t-u)P} f)` with `λ = c d_n + n R` has derivative
`e^{-λ(t-s)} m_s(G e^{(t-s)P} f)` at `s` within `S`: coalescence and the loss cancel, and only
branching into arity `n + 1` remains. -/
theorem hasDerivWithinAt_duhamel [Fintype H] [DecidableEq H] {E : Type*} [Fintype E] (c : ℝ)
    (r : E → ℝ) (T : E → H → H → H) (m : ℝ → (k : ℕ) → ((Fin k → H) → ℝ) →ₗ[ℝ] ℝ) (t s : ℝ)
    (S : Set ℝ) (hm : ∀ k (g : (Fin k → H) → ℝ),
      HasDerivWithinAt (fun u ↦ m u k g) (momentGenerator c r T (m s) k g) S s)
    (f : (Fin n → H) → ℝ) :
    HasDerivWithinAt
      (fun u ↦ Real.exp (-dualExitRate c r n * (t - u)) * m u n (gainSemigroup c (t - u) f))
      (Real.exp (-dualExitRate c r n * (t - s)) *
        m s (n + 1) (branchingGain r T (gainSemigroup c (t - s) f))) S s := by
  ...
  have hsum : HasDerivWithinAt
      ...
      (∑ w, (((-1 : ℝ) • coalescenceGain c (gainSemigroup c (t - s) f)) w *
        ...
        gainSemigroup c (t - s) f w *
          momentGenerator c r T (m s) n (fun j ↦ if w = j then 1 else 0))) S s :=
    HasDerivWithinAt.fun_sum fun w _ ↦ (HasFDerivAt.comp_hasDerivWithinAt
      (hl := (ContinuousLinearMap.proj (R := ℝ) (φ := fun _ : Fin n → H ↦ ℝ) w).hasFDerivAt)
      (hf := hv.hasDerivWithinAt)).mul (hm n _)
  have hvalue : ∑ w, (((-1 : ℝ) • coalescenceGain c (gainSemigroup c (t - s) f)) w *
    ...
    ring
  rw [hvalue] at hsum
  have hψ := hsum.congr_of_eventuallyEq (f₁ := fun u ↦ m u n (gainSemigroup c (t - u) f))
    (Filter.Eventually.of_forall fun u ↦ hdecomp u _) (hdecomp s _)
  ...
  convert hexp.hasDerivWithinAt.mul hψ using 1
  ring
...
theorem hasDerivAt_duhamel [Fintype H] [DecidableEq H] {E : Type*} [Fintype E] (c : ℝ)
    ...
    HasDerivAt
      (fun u ↦ Real.exp (-dualExitRate c r n * (t - u)) * m u n (gainSemigroup c (t - u) f))
      (Real.exp (-dualExitRate c r n * (t - s)) *
        m s (n + 1) (branchingGain r T (gainSemigroup c (t - s) f))) s :=
  hasDerivWithinAt_univ.mp
    (hasDerivWithinAt_duhamel c r T m t s Set.univ (fun k g ↦ (hm s k g).hasDerivWithinAt) f)

/-- **The Duhamel curve is continuous where the moments are**: if every moment `u ↦ m_u(g)` is
continuous on `S`, so is `u ↦ e^{-λ(t-u)} m_u(e^{(t-u)P} f)`. -/
theorem continuousOn_duhamel [Fintype H] [DecidableEq H] {E : Type*} [Fintype E] (c : ℝ)
    (r : E → ℝ) (m : ℝ → (k : ℕ) → ((Fin k → H) → ℝ) →ₗ[ℝ] ℝ) {S : Set ℝ}
    (hcont : ∀ k (g : (Fin k → H) → ℝ), ContinuousOn (fun u ↦ m u k g) S) (t : ℝ)
    (f : (Fin n → H) → ℝ) :
    ContinuousOn
      (fun u ↦ Real.exp (-dualExitRate c r n * (t - u)) * m u n (gainSemigroup c (t - u) f)) S := by
  have hdecomp : ∀ (u : ℝ) (x : (Fin n → H) → ℝ),
      m u n x = ∑ w, x w * m u n (fun j ↦ if w = j then 1 else 0) := fun u x ↦ by
    conv_lhs => rw [pi_eq_sum_univ x]
    simp only [map_sum, map_smul, smul_eq_mul]
  have hv : ∀ s, HasDerivAt (fun u ↦ gainSemigroup c (t - u) f)
      ((-1 : ℝ) • coalescenceGain c (gainSemigroup c (t - s) f)) s := fun s ↦
    HasDerivAt.scomp (hg := hasDerivAt_gainSemigroup_apply c (t - s) f)
      (hh := (hasDerivAt_id (x := s)).const_sub t)
  have hcoord : ∀ w, Continuous fun u ↦ gainSemigroup c (t - u) f w := fun w ↦
    continuous_iff_continuousAt.mpr fun s ↦ (HasFDerivAt.comp_hasDerivAt
      (hl := (ContinuousLinearMap.proj (R := ℝ) (φ := fun _ : Fin n → H ↦ ℝ) w).hasFDerivAt)
      (hf := hv s)).continuousAt
  have hfun : (fun u ↦ Real.exp (-dualExitRate c r n * (t - u)) *
      m u n (gainSemigroup c (t - u) f)) = fun u ↦ Real.exp (-dualExitRate c r n * (t - u)) *
        ∑ w, gainSemigroup c (t - u) f w * m u n (fun j ↦ if w = j then 1 else 0) :=
    funext fun u ↦ by rw [hdecomp u (gainSemigroup c (t - u) f)]
  rw [hfun]
  refine (Continuous.continuousOn (by fun_prop)).mul (continuousOn_finset_sum _ fun w _ ↦ ?_)
  exact (hcoord w).continuousOn.mul (hcont n _)

/-! ### The branching bound and uniqueness -/

/-- **Every branching costs a factor `R t`.** For a moment family with `|m_s(f)| ≤ K ‖f‖` at every
time `s ≥ 0`, moments continuous on `[0, ∞)` with right derivative `momentGenerator c r T m_s` at
every time `s ≥ 0`, and zero initial moments, `|m_t(f)| ≤ K ‖f‖ C(j + n, n) (R t)^j` at every time
`t ≥ 0` and for every number `j` of branchings. -/
theorem abs_moment_le_choose_mul_pow_right [Fintype H] [DecidableEq H] {E : Type*} [Fintype E]
    {c : ℝ} (hc : 0 ≤ c) {r : E → ℝ} (hr : ∀ e, 0 ≤ r e) (T : E → H → H → H)
    (m : ℝ → (k : ℕ) → ((Fin k → H) → ℝ) →ₗ[ℝ] ℝ) {K : ℝ} (hK : 0 ≤ K)
    (hbound : ∀ s, 0 ≤ s → ∀ k (g : (Fin k → H) → ℝ), |m s k g| ≤ K * ‖g‖)
    (hm : ∀ s, 0 ≤ s → ∀ k (g : (Fin k → H) → ℝ),
      HasDerivWithinAt (fun u ↦ m u k g) (momentGenerator c r T (m s) k g) (Set.Ici s) s)
    (hcont : ∀ k (g : (Fin k → H) → ℝ), ContinuousOn (fun u ↦ m u k g) (Set.Ici 0))
  ...
  | succ j ih =>
    ...
    have hmain := image_norm_le_of_norm_deriv_right_le_deriv_boundary
      ...
      (B' := fun u ↦ K * ‖g‖ * ((j + 1 + k).choose k : ℝ) * (∑ e, r e) ^ (j + 1) *
        (((j + 1 : ℕ) : ℝ) * u ^ j))
      ((continuousOn_duhamel c r m hcont t g).mono fun u hu ↦ hu.1)
      (fun u hu ↦ hasDerivWithinAt_duhamel c r T m t u (Set.Ici u) (hm u hu.1) g)
      (by simp [h0])
      (fun u ↦ by
...
theorem abs_moment_le_choose_mul_pow [Fintype H] [DecidableEq H] {E : Type*} [Fintype E]
    ...
    (h0 : ∀ k (g : (Fin k → H) → ℝ), m 0 k g = 0) (j : ℕ) :
    ∀ (k : ℕ) (g : (Fin k → H) → ℝ) {s : ℝ}, 0 ≤ s →
      |m s k g| ≤ K * ‖g‖ * (((j + k).choose k : ℝ) * ((∑ e, r e) * s) ^ j) :=
  abs_moment_le_choose_mul_pow_right hc hr T m hK (fun s _ ↦ hbound s)
    (fun s _ k g ↦ (hm s k g).hasDerivWithinAt)
    (fun k g ↦ (continuous_iff_continuousAt.mpr fun s ↦ (hm s k g).continuousAt).continuousOn)
    h0 j

/-- **A family starting at zero stays at zero while `R t < 1`**, from right derivatives at times
`s ≥ 0`. -/
theorem moment_eq_zero_of_mul_lt_one_right [Fintype H] [DecidableEq H] {E : Type*} [Fintype E]
    {c : ℝ} (hc : 0 ≤ c) {r : E → ℝ} (hr : ∀ e, 0 ≤ r e) (T : E → H → H → H)
    (m : ℝ → (k : ℕ) → ((Fin k → H) → ℝ) →ₗ[ℝ] ℝ) {K : ℝ} (hK : 0 ≤ K)
    (hbound : ∀ s, 0 ≤ s → ∀ k (g : (Fin k → H) → ℝ), |m s k g| ≤ K * ‖g‖)
    (hm : ∀ s, 0 ≤ s → ∀ k (g : (Fin k → H) → ℝ),
  ...
  have hle : |m s k g| ≤ 0 := ge_of_tendsto' htends fun j ↦
    abs_moment_le_choose_mul_pow_right hc hr T m hK hbound hm hcont h0 j k g hs
...
theorem moment_eq_zero_of_mul_lt_one [Fintype H] [DecidableEq H] {E : Type*} [Fintype E]
    ...
    (hm : ∀ s k (g : (Fin k → H) → ℝ),
      HasDerivAt (fun u ↦ m u k g) (momentGenerator c r T (m s) k g) s)
    (h0 : ∀ k (g : (Fin k → H) → ℝ), m 0 k g = 0) {s : ℝ} (hs : 0 ≤ s)
    (hRs : (∑ e, r e) * s < 1) (k : ℕ) (g : (Fin k → H) → ℝ) : m s k g = 0 :=
  moment_eq_zero_of_mul_lt_one_right hc hr T m hK (fun s _ ↦ hbound s)
    (fun s _ k g ↦ (hm s k g).hasDerivWithinAt)
    (fun k g ↦ (continuous_iff_continuousAt.mpr fun s ↦ (hm s k g).continuousAt).continuousOn)
    h0 hs hRs k g

/-- **A family starting at zero stays at zero**, from right derivatives at times `s ≥ 0`. Shifting
time by multiples of `1/(R + 1)` extends the short-horizon statement to every `t ≥ 0`. -/
theorem moment_eq_zero_right [Fintype H] [DecidableEq H] {E : Type*} [Fintype E] {c : ℝ}
  ...
  have hshift : ∀ j : ℕ, ∀ s : ℝ, 0 ≤ s → s ≤ j * (1 / (∑ e, r e + 1)) →
    ...
    | succ j ih =>
      ...
      · push_neg at hsj
        ...
        have hmshift : ∀ u : ℝ, 0 ≤ u → ∀ (k : ℕ) (g : (Fin k → H) → ℝ),
            HasDerivWithinAt (fun v ↦ m (j * (1 / (∑ e, r e + 1)) + v) k g)
              (momentGenerator c r T (m (j * (1 / (∑ e, r e + 1)) + u)) k g) (Set.Ici u) u :=
          fun u hu k g ↦ by
            have hadd : HasDerivWithinAt (fun v ↦ (j : ℝ) * (1 / (∑ e, r e + 1)) + v) 1
                (Set.Ici u) u :=
              ((hasDerivAt_id (x := u)).const_add _).hasDerivWithinAt
            have h := HasDerivWithinAt.comp (hh₂ := hm _ (add_nonneg hj0 hu) k g) (hh := hadd)
              (hst := fun v hv ↦ by
                simp only [Set.mem_Ici] at hv ⊢
                linarith)
            simpa using h
        have hcshift : ∀ (k : ℕ) (g : (Fin k → H) → ℝ),
            ContinuousOn (fun v ↦ m (j * (1 / (∑ e, r e + 1)) + v) k g) (Set.Ici 0) :=
          fun k g ↦ (hcont k g).comp' (Continuous.continuousOn (by fun_prop)) fun v hv ↦ by
            simp only [Set.mem_Ici] at hv ⊢
            linarith
        have hzero := moment_eq_zero_of_mul_lt_one_right hc hr T
          (fun u ↦ m (j * (1 / (∑ e, r e + 1)) + u)) hK
          (fun u hu k g ↦ hbound _ (add_nonneg hj0 hu) k g) hmshift hcshift
...
/-- **A family starting at zero stays at zero**, for a family with a derivative at every real
time. -/
theorem moment_eq_zero [Fintype H] [DecidableEq H] {E : Type*} [Fintype E] {c : ℝ} (hc : 0 ≤ c)
    ...
    (hm : ∀ s k (g : (Fin k → H) → ℝ),
      HasDerivAt (fun u ↦ m u k g) (momentGenerator c r T (m s) k g) s)
    (h0 : ∀ k (g : (Fin k → H) → ℝ), m 0 k g = 0) {t : ℝ} (ht : 0 ≤ t) (k : ℕ)
    (g : (Fin k → H) → ℝ) : m t k g = 0 :=
  moment_eq_zero_right hc hr T m hK (fun s _ ↦ hbound s)
    (fun s _ k g ↦ (hm s k g).hasDerivWithinAt)
    (fun k g ↦ (continuous_iff_continuousAt.mpr fun s ↦ (hm s k g).continuousAt).continuousOn)
    h0 ht k g

/-- **The duality (7.5) with decisions, in moment form, from right derivatives.** Two families of
moment functionals on the observations of every arity, bounded by the sup norm at every time
`s ≥ 0`, continuous on `[0, ∞)` and obeying the moment equation of the backward circuit as a right
derivative at every time `s ≥ 0`, that agree at time zero agree at every time `t ≥ 0`. -/
theorem moments_eq_of_rightMomentEquation [Fintype H] [DecidableEq H] {E : Type*} [Fintype E]
    {c : ℝ} (hc : 0 ≤ c) {r : E → ℝ} (hr : ∀ e, 0 ≤ r e) (T : E → H → H → H)
    (m m' : ℝ → (k : ℕ) → ((Fin k → H) → ℝ) →ₗ[ℝ] ℝ)
    (hbound : ∀ s, 0 ≤ s → ∀ k (g : (Fin k → H) → ℝ), |m s k g| ≤ ‖g‖)
    (hbound' : ∀ s, 0 ≤ s → ∀ k (g : (Fin k → H) → ℝ), |m' s k g| ≤ ‖g‖)
    (hm : ∀ s, 0 ≤ s → ∀ k (g : (Fin k → H) → ℝ),
      HasDerivWithinAt (fun u ↦ m u k g) (momentGenerator c r T (m s) k g) (Set.Ici s) s)
    (hm' : ∀ s, 0 ≤ s → ∀ k (g : (Fin k → H) → ℝ),
  ...
  have hdiff := moment_eq_zero_right hc hr T (fun s k ↦ m s k - m' s k) (K := 2) (by norm_num)
    (fun s hs k g ↦ by
      show |m s k g - m' s k g| ≤ 2 * ‖g‖
      have h1 := abs_le.mp (hbound s hs k g)
      have h2 := abs_le.mp (hbound' s hs k g)
      exact abs_le.mpr ⟨by linarith [h1.1, h2.2], by linarith [h1.2, h2.1]⟩)
    (fun s hs k g ↦ by
      have h := (hm s hs k g).sub (hm' s hs k g)
      rw [← momentGenerator_sub] at h
      exact h)
    (fun k g ↦ by
      show ContinuousOn (fun u ↦ m u k g - m' u k g) (Set.Ici 0)
      exact (hcont k g).sub (hcont' k g))
    (fun k g ↦ by
...
theorem moments_eq_of_momentEquation [Fintype H] [DecidableEq H] {E : Type*} [Fintype E] {c : ℝ}
  ...
  moments_eq_of_rightMomentEquation hc hr T m m' (fun s _ ↦ hbound s) (fun s _ ↦ hbound' s)
    (fun s _ k g ↦ (hm s k g).hasDerivWithinAt) (fun s _ k g ↦ (hm' s k g).hasDerivWithinAt)
    (fun k g ↦ (continuous_iff_continuousAt.mpr fun s ↦ (hm s k g).continuousAt).continuousOn)
    (fun k g ↦ (continuous_iff_continuousAt.mpr fun s ↦ (hm' s k g).continuousAt).continuousOn)
    h0 ht k g
