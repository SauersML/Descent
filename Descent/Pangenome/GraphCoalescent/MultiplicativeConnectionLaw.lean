...
def kernelMapEquiv {w x : ℕ} (σ : ER w) :
  ...
  right_inv g := Function.Embedding.ext fun c ↦ Quotient.inductionOn c fun _ ↦ rfl
...
def blockLabelEquiv {w x : ℕ} (τ : ER w) :
  ...
  left_inv f := Subtype.ext rfl
  right_inv g := funext fun c ↦ Quotient.inductionOn c fun _ ↦ rfl
...
def componentPartition {w : ℕ} (G : FiberPair w → Bool) : ER w :=
  EqvGen.setoid (configAdjacent G)
