...
from strong lumpability in Rosenblatt's form, the form used in `LumpingVisibleRates`.

`card_covers_eq_of_lumping` shows that two states with the same value of a strong lumping have
equally many covers. A state with `K` true blocks has `C(K, 2)` covers, so `blocks_eq_of_lumping`
concludes that the two states have the same number of true blocks. With two report components the
two loads add up to that number (`hiddenLoad_add_hiddenLoad`), so the statistic determines
`a + b`, and `LumpingVisibleRates.visibleRate_eq_of_lumping` gives `ab`. Sum and product fix both
survival derivatives (`survivalDerivatives_eq_of_sum_product`), which is
...
/-- **A strong lumping counts all covers alike.** Two states with the same value of a strong
lumping have equally many covers. Assumes: the statistic is a strong lumping in Rosenblatt's
form. -/
theorem card_covers_eq_of_lumping {Stat : Type*} (f : ER n → Stat)
    (hlumping : ∀ ξ ξ' : ER n, f ξ = f ξ' → ∀ v,
      Nat.card {η : ER n // Covers ξ η ∧ f η = v} = Nat.card {η : ER n // Covers ξ' η ∧ f η = v})
    {ξ ξ' : ER n} (hsame : f ξ = f ξ') :
  ...
  refine card_filter_comp_eq _ _ f (fun _ ↦ ()) (fun v ↦ ?_) ()
...
`C(K, 2)` covers, so two states with the same value of a strong lumping have the same number of
true ancestral blocks. Assumes: the statistic is a strong lumping in Rosenblatt's form. -/
theorem blocks_eq_of_lumping {Stat : Type*} (f : ER n → Stat)
    (hlumping : ∀ ξ ξ' : ER n, f ξ = f ξ' → ∀ v,
      Nat.card {η : ER n // Covers ξ η ∧ f η = v} = Nat.card {η : ER n // Covers ξ' η ∧ f η = v})
    (hn : 0 < n) {ξ ξ' : ER n} (hsame : f ξ = f ξ') : blocks ξ = blocks ξ' := by
  haveI : NeZero n := ⟨hn.ne'⟩
  have hcovers : (blocks ξ).choose 2 = (blocks ξ').choose 2 := by
    rw [← card_covers ξ, ← card_covers ξ']
    exact card_covers_eq_of_lumping f hlumping hsame
...
theorem survivalDerivatives_eq_of_lumping {Stat : Type*} (s : Fin n → Fin n) (f : ER n → Stat)
  ...
  exact blocks_eq_of_lumping f hlumping (Fin.pos x) hsame
