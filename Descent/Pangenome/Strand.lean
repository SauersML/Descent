...
/-- **The presentation generated by oriented alignments.**  Its positions are doubled by
strand, and its coordinates are the equivalence classes forced by the oriented alignment
relation.  This places the strand obstruction inside the same presentation category as the
unoriented pangenome construction.

Empirical status: NOT AN EMPIRICAL CLAIM. -/
def strandPresentation {Pos : Type*} (S : Pos → Pos → Bool → Prop) :
    Presentation (Pos × Bool) :=
  Presentation.closurePresentation (doubled S)
...
/-- **Frustration is identification with one's reverse complement in the universal
presentation.**  This is the categorical form of the obstruction: the generated quotient
has collapsed the two orientations of some observed position into one coordinate. -/
theorem frustrated_iff_encode_eq {Pos : Type*} (S : Pos → Pos → Bool → Prop) :
    Frustrated S ↔
      ∃ p : Pos, (strandPresentation S).encode (p, false) =
        (strandPresentation S).encode (p, true) := by
  apply exists_congr
  intro p
  change EqvGen (doubled S) (p, false) (p, true) ↔
    (strandPresentation S).kernel (p, false) (p, true)
  rw [strandPresentation, Presentation.closurePresentation, Presentation.kernel_ofSetoid]
  exact (Construction.closure_rel_iff (doubled S) (p, false) (p, true)).symm
