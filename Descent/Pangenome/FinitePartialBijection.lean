...
`NonsoficGroupsExist/Matching/FinitePartialBijection.lean`.
-/
import Descent.Pangenome.FiniteModel
...
noncomputable instance finite : Finite (FinitePartialBijection Y Z) := by
  ...
  letI : Fintype (Finset Y × Finset Z × Finset (Y × Z)) := inferInstance
  ...
  have hbmem : (y, b.apply y hy) ∈
      (encode b).2.2 := by
    simp [encode, hy]
  have hcmem : (y, b.apply y hy) ∈ (encode c).2.2 := by
    change (y, b.apply y hy) ∈ (fun p ↦ p.2.2) (encode c)
    rw [← hg]
    exact hbmem
...
theorem exists_extension (b : FinitePartialBijection Y Y) :
  ...
  classical
  refine ⟨b.equiv.extendSubtype, fun y hy ↦ ?_⟩
  exact Equiv.extendSubtype_apply_of_mem b.equiv y hy
