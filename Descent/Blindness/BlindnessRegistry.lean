...
`Descent.Blindness.ObservationalCeiling` states the law: a probe that reports the same data on
...
(`validation/empirical/simcov/battery_blind.py`), the raw mean pairwise time spans
...
`validation/empirical/simcov` is `1 - E[T_within] / E[T_between]`, a ratio of
...
Guard 3j of `validation/code/check.py` (declare your regime) meets it — the two
regimes of `Descent.PopGen.DriftRegime` are separated as objects, and a regime declaration is
...
inductive StructuralGuard
  ...
  fails to compile (`Descent.Foundations.Conventions`). -/
  | overDetermination
...
midpoint.
    ...
    Empirical status: NOT AN EMPIRICAL CLAIM. Definitional within the model
    declared above: it fixes a contrast rather than predicting an observable.
