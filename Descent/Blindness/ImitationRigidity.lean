...
`Descent.PCCorrectability.ImitationCapacity` proves it in that generality,
...
analogue of the Ohta–Kimura decay in `Descent.PopGen.LDDecayTheory` — makes the
...
theorem stationaryLDEntry_eq_ldAfterGenerations (r Ne : ℝ) (separation : ℕ) :
    stationaryLDEntry (PopGen.ldRetentionPerGen r Ne) separation =
      PopGen.ldAfterGenerations 1 r Ne separation := by
  unfold stationaryLDEntry PopGen.ldAfterGenerations Descent.Core.power
  rw [one_mul]
...
`Descent.PopGen.LDDecayTheory` makes the detection-relevant spectral quantity an
...
theorem ldWhiteningGain_of_ldRetention_antitone
    ...
    ldWhiteningGain (PopGen.ldRetentionPerGen r₂ Ne) <
      ldWhiteningGain (PopGen.ldRetentionPerGen r₁ Ne) := by
  have hret₂ : 0 ≤ PopGen.ldRetentionPerGen r₂ Ne :=
    PopGen.ld_retention_nonneg r₂ Ne (le_of_lt hr₂) (le_of_lt hNe)
  have hlt' : PopGen.ldRetentionPerGen r₂ Ne < PopGen.ldRetentionPerGen r₁ Ne := by
    unfold PopGen.ldRetentionPerGen
    have hfac : 0 < 1 - 1 / (2 * Ne) := by
      rw [sub_pos, div_lt_one (by linarith)]; linarith
    nlinarith [hlt, hfac]
  have hone : PopGen.ldRetentionPerGen r₁ Ne < 1 := by
    ...
    · unfold PopGen.ldRetentionPerGen
      ...
      have hfac : 1 - 1 / (2 * Ne) < 1 := by
        ...
        linarith
      nlinarith [hfac]
    · exact PopGen.ld_retention_lt_one r₁ Ne hpos (by linarith) hNe
  have habs : |PopGen.ldRetentionPerGen r₁ Ne| < 1 := by
    rw [abs_lt]
    constructor
    · nlinarith [PopGen.ld_retention_nonneg r₁ Ne (by linarith) (le_of_lt hNe)]
    · exact hone
...
`validation/empirical/imitation_rigidity/check_imitation.py` records.
...
* the imitation bump acts on the normal equations of `Descent.Spectral.SecondMomentShift`
  through a single scalar, so a residual-score analysis cannot separate it from
  a background;
* whitening — the operation `Descent.Spectral.WhiteningEquivalence` formalizes — maps
...
* the dropout floor of the dead-sensor construction caps the fraction of
  ...
  is the mechanism behind open question 1 of `Descent.Program.OpenQuestions`.
-/
...
to the signal.** The second-moment matrix of `Descent.Spectral.SecondMomentShift`
...
right-side transform of `Descent.Spectral.WhiteningEquivalence` to a signal-carrying
