...
`Descent.Blindness.CountingInvariantBlindness` states the general phenomenon; this module supplies
...
Simulation in exact rational arithmetic (`validation/empirical/meff_error_floor/`, `n` up to
32,
...
`validation/empirical/meff_error_floor/`. The positive control passes: the
