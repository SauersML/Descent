...
formalized in `validation/conventions.json`: what one must inject is a section of the
...
`Descent.Blindness.ObservationalCeiling`: membership is witnessed by a single natural number,
