...
The instances live in `Descent.Blindness.BlindnessRegistry`, which is the single place they are
...
instances is in `Descent.Blindness.BlindnessRegistry`; it is deliberately not repeated here, so
...
`Descent.Foundations.Conventions` already states for constants, extended here to structural
quantities.
-/
...
Every instance in `Descent.Blindness.BlindnessRegistry` is exact. `same_data` is an equality, so
...
`Descent.BlindnessRegistry.averageEffect_separation` is the instance with a genotypic
...
`Descent.duplicate_separation` is not a proposition". The keyword is the only thing at
