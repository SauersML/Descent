...
* `(c, v)` via the location of the condensation threshold and the window profile
  (`Descent.Blindness.Condensation`);
...
* To our knowledge the *completeness* formulation — quantifying over designs rather
  ...
  the mechanism, which is theirs (see `Descent.Blindness.Condensation` for the ledger).
...
`Descent.Spectral.PolygenicSpectroscopy`.
...
* **Symmetry-free.** The lattice arithmetic (`one_lt_latticeInflation`,
  ...
  of `Descent.Blindness.Condensation`. These are the parts that apply to genotypes at
  every allele frequency — and they are the parts
  `Descent.Spectral.PolygenicSpectroscopy` actually instantiates.

`Descent.Spectral.PolygenicSpectroscopy` instantiates the symmetry-free side only. The split is
...
* **disjoint variable supports**, i.e.
  `Descent.EpistaticChaos.GenotypeDesign.VariantDisjoint` — non-overlapping genes,
...
* **sign symmetry of the coordinate law**, which for a standardized Hardy-Weinberg
  ...
  `Descent.PolygenicSpectroscopy.hweMellinJetVariance_half` gives `v(1/2) = 0`, so
...
drift/condensation machinery of `Descent.Blindness.Condensation`.
-/
...
`Descent.Spectral.PolygenicSpectroscopy` states its hard-call comparison in this vocabulary. -/
