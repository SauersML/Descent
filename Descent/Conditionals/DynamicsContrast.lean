...
* `Σ(x)` is observable genotype geometry.  Its generalized eigenvalues are handled by
  `Descent.Spectral.ErgodicCovariancePencil`.
* `θ(x)` is context-specific biological value.  The cost of adapting a readout to it is
  handled by `Descent.Spectral.DirichletTransfer`.
* A probe exposes the parameter only modulo a declared nuisance class.  Identification is
  governed by `Descent.Conditionals.DeclaredInteractionClass`, independently of the transport and
  pencil calculations.
...
`Descent.Conditionals.DescentGeometry`: a cross-state criterion is not a function of the target context,
