...
The instances live in `Descent.BlindnessRegistry`, which is the single place they are
...
instances is in `Descent.BlindnessRegistry`; it is deliberately not repeated here, so
...
`Descent.Conventions` already states for constants, extended here to structural
quantities.
-/
...
Every instance in `Descent.BlindnessRegistry` is exact. `same_data` is an equality, so
