...
`Descent.ErgodicCovariancePencil` builds source and target covariance operators as two time
...
`Descent.HiddenConeAmbiguity` answers the abstract version — the gauge freedom is large and
...
`Descent.DeclaredInteractionClass` dichotomy in demography: the
