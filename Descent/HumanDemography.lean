...
Measured in `validation/empirical/drift_diff/`. Use `taggedDriftR2RatioCorrected` below. -/
...
size, migration rate, shared tagging. `Descent.LumpedRateBlindness` marks off a parameter that
...
This is `Descent.DeclaredInteractionClass` in demography: identification here is relative to a
