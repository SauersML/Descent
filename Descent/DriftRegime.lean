...
`Descent.Conventions` states the standing defence:
...
(`Descent.ObservationalCeiling`), applied to the development's own quality process.
...
immediately below.
    ...
    measurement it explains is recorded in `Descent.PortabilityDrift`. -/
...
The deeper point, and the reason this file sits next to `Descent.ObservationalCeiling`
