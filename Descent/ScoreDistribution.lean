...
`Descent.Probability` holds the one body for it. A second spelling in this file, such
...
`Descent.Condensation` fixes the price of leaving that regime. A score aggregating terms
...
where it does not shrink. `Descent.JetBarrier` supplies the missing factor.
