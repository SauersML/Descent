...
`Descent.PolygenicSpectroscopy` instead.
...
condensation/drift arc: the direct quantities in `Descent.Condensation` require no
...
`Descent.PolygenicSpectroscopy`, `log x²` is supported on a single point and
...
point live in `Descent.LDDecayTheory`.
