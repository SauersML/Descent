...
at the size-bias point, and by `Descent.Condensation` it is exactly the quantity
...
`Descent.ScoreDistribution` applies at `m = 1`. Define
...
*first* observable is blind: `Descent.Condensation`'s drift-separation theorem has
...
*do* see it is the trichotomy conjecture of `Descent.JetBarrier`, which that file
...
`Descent.HiddenConeAmbiguity` shows that the decay profile of latent loadings is
...
`Descent.Conventions`, and `Descent.PCCorrectability` quantifies what correction
...
`Descent.Probability`, which this module imports transitively.  A second copy
...
mean of `log x ^ 2` under the size-biased law. By `Descent.Condensation` this is
...
`Descent.Condensation` already separates a balanced hard-called locus from its
...
`Descent.JetBarrier` conjectures to be observable. A hard-called genotype has
...
design observes — is the `Descent.JetBarrier` conjecture, which is not proved.
-/
...
one frequency where the symmetric-law results of `Descent.JetBarrier` may be
...
condensation-window theory of `Descent.Condensation` — which needs a non-degenerate
...
`Descent.CramerStratum` proves that a lattice coordinate law violates Cramér's
...
The load-bearing consequence, and the reason `Descent.CramerStratum` is imported by
...
* `Descent.ScoreDistribution` and the Berry-Esseen certificates of
  `Descent.Probability` are **untouched**: they concern degree-one aggregates,
...
* `Descent.EpistasisAndNonAdditivity` is where the boundary bites. Its
...
* `Descent.ImputationPortability` gains a second, non-additive mechanism: beyond
...
* `Descent.PCCorrectability` is unaffected in content and sharpened in status: it
  answers "what does correction achieve given a convention", and
  `Descent.HiddenConeAmbiguity` proves the convention cannot be replaced by an
  inference.
* Summary-statistic coherence checks are bounded-radius audits, and
  `Descent.LocalToGlobalCoherence` proves no such audit certifies joint
...
and the direct formulas in `Descent.Condensation` require no symmetry field. That is
