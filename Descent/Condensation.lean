...
assumption underlying `Descent.ScoreDistribution`, liability-threshold
calibration, the Berry-Esseen certificates in `Descent.Probability` — is an
...
is formalized in `Descent.PolygenicSpectroscopy`; this is the bridge sentence.
...
function of allele frequency in `Descent.PolygenicSpectroscopy`. It is **not**
