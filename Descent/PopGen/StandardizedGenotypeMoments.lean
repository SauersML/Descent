...
`Descent.Foundations.Conventions` states the standing policy for exactly that situation:
...
in the sense of `Descent.Foundations.Conventions`; `Descent.Portability.PCCorrectability` answers the
...
buys. That is the programme `Descent.Foundations.Conventions` already states, and the three
...
theorem twoMechanismMixture_eq_convexMix (w : ℝ) :
    twoMechanismMixture w = Descent.Core.convexCombination w (2 / 10) (9 / 10) := rfl
