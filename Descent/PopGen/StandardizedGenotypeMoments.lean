...
* `mellinDrift_uses_ploidy` — the standardization inside the Mellin drift is the
  *same* `ploidy` constant as the rest of the corpus. If anyone changes the factor of
  two in `Descent.Conventions.ploidy`, or writes `q (1 - q)` where the corpus means
...
This is the precise reason the lattice discrepancy of `Descent.Blindness.JetBarrier` is not a
restatement of the `r ^ 2` attenuation modelled in `Descent.Portability.ImputationPortability`:
...
`Descent.Foundations.Probability` describe degree-one aggregates. Those are deeply subcritical:
...
degree one is the Berry-Esseen apparatus of `Descent.Foundations.Probability`, not this. Genome-wide
...
`Descent.Portability.ImputationPortability` models the dosage/hard-call difference as an
...
`Descent.EpistaticChaos.sign_erasure` shows that when a coding is **sign-symmetric**
...
`Descent.Blindness.JetBarrier` could be quoted for a genome, and *both* fail.
...
2. Sign symmetry would have to imply that overlapping designs behave like disjoint
   ...
   design: `Descent.EpistaticChaos.sign_symmetry_does_not_license_disjoint_reduction`
...
`Descent.EpistaticChaos.GenotypeDesign.VariantDisjoint`, and no property of the
...
tempered class of `Descent.EpistaticChaos.GenotypeDesign.Tempered`, and in that phase
...
(`Descent.EpistaticChaos.centeredSquare_rademacher_at_half`). Away from `1/2` the
...
`Descent.EpistaticChaos.uncentered_square_log_additive`: the uncentered square recycles
...
theorem standardizedSquare_never_symmetric (h : HardyWeinbergModel)
  ...
  have hzero : ∑ g : Foundations.DiploidGenotype,
...
noncomputable def hweLevelOne (q : ℝ) : LevelChannels := by
  classical
  exact
    { drift := Spectral.hweMellinDrift q
      jetVariance := Spectral.hweMellinJetVariance q
      isLattice := decide (Spectral.hweLatticeCondition q)
...
theorem hweLevelOne_components (q : ℝ) :
    (hweLevelOne q).drift = Spectral.hweMellinDrift q ∧
      (hweLevelOne q).jetVariance = Spectral.hweMellinJetVariance q ∧
      ((hweLevelOne q).isLattice = true ↔ Spectral.hweLatticeCondition q) ∧
      ((hweLevelOne q).isSignSymmetric = true ↔ q = 1 / 2) := by
  classical
  refine ⟨rfl, rfl, ?_, ?_⟩
  · change decide (Spectral.hweLatticeCondition q) = true ↔ Spectral.hweLatticeCondition q
...
theorem hweLevelOne_symmetric_jetVariance_zero (q : ℝ)
  ...
  have hjet : (hweLevelOne q).jetVariance = Spectral.hweMellinJetVariance q := rfl
  rw [hjet, hcomponent]
  exact Spectral.hweMellinJetVariance_half
...
(`Descent.EpistaticChaos.centeredSquare_third_moment_eq`), and the trap beside it —
...
in this repository**. There is no such theorem in `Descent`; the claimed
mechanism is a contraction argument, and it is cited here, as it is in
`Descent.Spectral.PolygenicSpectroscopy` §4c, only to name the hypotheses being ruled out.
...
(`Descent.EpistaticChaos.centeredSquare_rademacher_at_half`). Away from `q = 1/2` the
...
the law is probed. That is what `Descent.Blindness.JetBarrier` had already said, so the arc
...
* `Descent.EpistaticChaos.hweSignBias_eq` — `E[x|x|] = (1-2q)²` for `q ≤ 1/2`. This is
...
(`validation/empirical/coupling/ladder_moments.py`) confirms the sharp form the bound only
...
`Descent.Blindness.JetBarrier` proves `one_lt_latticeInflation`: the factor
...
`validation/empirical/coupling/fiber_splitting.py` over uniform, rare-weighted, clustered
...
`2/10` and `9/10` by `w` and `1 - w`, which is `Core.convexCombination` at those
