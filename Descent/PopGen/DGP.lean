...
calls the kernel.  Its VALIDATED record moves with it, to `Core/Fst.lean`, because evidence belongs
on the body that was measured and not on a name that forwards to it. -/
...
Two accessors on two structures used to write this product out separately.
    ...
    gap stated (`validation/empirical/simcov/battery_max.py`,
...
Empirical status: **VALIDATED** (`validation/empirical/simcov/battery_dis4.py`).
...
/-- **The level's `4 Nₑ mTot` is `scaledMigrationRate` at the total emigration rate**, not a
free coefficient. The corpus's scaled migration rate is `2 · ploidy · Nₑ · m`, so writing the
migration-drift balance at the row sum rather than at a per-neighbour rate changes WHICH rate
is scaled and leaves the scaling convention alone. -/
theorem fstConnectedPairAt_uses_scaledMigrationRate (Ne : ℝ) (θ : Descent.Core.Theta)
    (mTot : ℝ) (t : ℕ) :
    fstConnectedPairAt Ne θ mTot t =
      (1 / (1 + θ.value + Descent.Core.scaledMigrationRate Ne mTot)) *
        (1 - (hetDecayFromScaled Ne θ * (1 - mTot)) ^ t) := by
  unfold fstConnectedPairAt Descent.Core.scaledMigrationRate Descent.Core.ploidy
  ring_nf
...
/-! ### Study-design information for a single cohort -/
...
`w*_S = Σ_tag,S^{-1} Σ_tc,S β_c`.
    ...
    Empirical status: **VALIDATED**
    (`validation/empirical/simcov/battery_dis2.py`). 40000 individuals,
...
theorem target_r2_drop_of_migration_gap_of_params
  ...
  target_r2_drop_of_fst_and_sparse_array mseSource mseTarget (source.V_A + V_E) lam
    ...
    (target.fstEquilibrium_lt_of_mig_lt source hNe hmu hd hmig)
...
`Descent.SpectralDegradation.FiniteSpectralModel.degradation_eq_zero_iff` is exactly
...
Deleting `Descent.Spectral.SpectralDegradation` breaks this theorem:
...
convention restated where a proof can check it.
    ...
    Regime: an ancestry-indexed LD environment at fixed allele frequencies, with selection entering
    as an additive displacement of the between-population
...
`Descent.ld_decay_implies_nonlinear_calibration_of_exp_tagging` in the corpus root, which
supplies
...
`Descent.ld_decay_implies_nonlinear_calibration_of_exp_tagging`, which exhibits three
...
above lives in `Descent.lean` -- the corpus ROOT, one level *above*
`Descent/`. A dead-code scan that walks only `Descent/` cannot see it,
...
`Descent.ld_decay_implies_nonlinear_calibration_of_exp_tagging`, which lives in
`Descent.lean` — the corpus ROOT, one directory *above* `Descent/`.
A dead-code scan that walks only `Descent/` reports this and `decaySlope` as
...
fixed both would tie a general statement to one design.
    ...
    Empirical status: UNTESTED. An assumed functional form for LD decay with distance; the corpus
    result it feeds is about the SHAPE being non-affine,
...
`Descent.ld_decay_implies_nonlinear_calibration_of_exp_tagging` requires of
...
/-- **Drift-migration equilibrium Fst**: Fst = 1/(1 + M).
    ...
    (`validation/empirical/simcov/battery_bulk18b.py`). Island model with the TOTAL emigration rate
    held fixed at `4 Ne m = 2.0`, deme count swept,
...
identical by descent.  In one generation drift makes a pair identical with probability `1/(2 Nₑ)`
among the pairs that are not already identical, and each
...
This was `harmonic_mean_governs_drift`, documented as "Fst ≈ 1 - exp(-T/(2 Ne_h))" with bottlenecks
"disproportionately increasing Fst".  No Fst, no exponential and no drift
...
/-- **AUC of the equal-variance Gaussian model**, from signal and residual
    ...
    **Not the liability-threshold AUC.** The docstring here used to call it
    ...
    The AUC is COUNTED, not computed: two Gaussian score distributions with equal variance and a
    mean separation of `√vSignal`, and the observable is
...
chart is a derived algebraic identity.
    ...
    **The in-regime agreement is AN IDENTITY, not evidence.** `Brier = E[p(1-p)]
    ...
    Where it is exact. If the calibrated probability `p` is itself a random variable with `E[p] = π`
    and `Var(p) = σ²`, then `Brier = E[p(1-p)] = π(1-π) - σ²` identically, so the
...
LIABILITY-scale explained fraction. This is what `calibratedBrierFromVariances` is not:
that one holds when `r²` is the observed-scale explained fraction and is wrong by 9% to 47%
when the outcome is a truncated liability tail, and the difference is the scale rather than
the simulation.

NOTHING BELOW CLAIMS THIS IS RIGHT FOR ANY TRAIT. What it claims is that IF a trait is
generated by a Gaussian liability crossing a threshold, THEN this is the Brier risk that
model implies, written analytically instead of charted. Whether that model describes a given
trait is an empirical question this declaration does not answer, and is what its UNTESTED
head is about.
...
/-- **Allele-frequency divergence rate: the drift rate, and only the drift rate.**
    ...
    **The body has been corrected and the arguments are now inert.** It read
    ...
    (`validation/empirical/simcov/battery_dis1.py`). Across-replicate
