...
theorem cohortShift_at_reference_point {n : ℕ} (i : Fin n) :
    cohortShift ⟨fun _ ↦ 1, fun _ ↦ 2⟩ 3 5 i = 13 := by
  unfold cohortShift
  norm_num
...
`Descent.Spectral.FoldedSpectrum` does the rest: the whole collinear family shares its two
...
`Descent.Conditionals.LatentMechanismCollapse` proves the finite shadow of the collapse theorem --
