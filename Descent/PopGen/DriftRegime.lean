...
assert_below Descent.Spectral Descent.Conditionals Descent.Portability Descent.Decision
assert_below Descent.Program

-- LAYER DEBT. This file cannot yet assert it is below `Descent.Blindness`:
--   Blindness: reaches 1 module(s) -- `Descent.Blindness.ObservationalCeiling`
...
`validation/conventions.json` states the standing defence:
...
`validation/code/check.py` now enforces it.
-/
...
(`Descent.Blindness.ObservationalCeiling`), applied to the development's own quality process.
...
/-- The neutral allele-frequency benchmark ratio, in the form under test.
    ...
    * on a symmetric design, `0.000%`.
    ...
    prediction. See `validation/empirical/drift_diff/`. -/
...
/-- **The benchmark ratio at a fully differentiated source, named.** A source population with `Fst =
1` is fixed, so the benchmark it provides is degenerate and the ratio diverges. The divisor
...
1. **A regime is a hypothesis, not a body.** A definition whose value depends on an
   ...
   positive time. `validation/code/check.py` guard 3i requires a `Regime:` line
...
The deeper point, and the reason this file sits next to `Descent.Blindness.ObservationalCeiling`
