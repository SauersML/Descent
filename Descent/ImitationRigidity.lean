...
analogue of the Ohta–Kimura decay in `Descent.LDDecayTheory` — makes the
...
`Descent.LDDecayTheory` makes the detection-relevant spectral quantity an
...
* the imitation bump acts on the normal equations of `Descent.SecondMomentShift`
  through a single scalar, so a residual-score analysis cannot separate it from
  a background;
* whitening — the operation `Descent.WhiteningEquivalence` formalizes — maps
...
* the dropout floor of the dead-sensor construction caps the fraction of
  ...
  is the mechanism behind open question 1 of `Descent.OpenQuestions`.
-/
...
to the signal.** The second-moment matrix of `Descent.SecondMomentShift`
...
right-side transform of `Descent.WhiteningEquivalence` to a signal-carrying
