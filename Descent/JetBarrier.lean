...
* To our knowledge the *completeness* formulation — quantifying over designs rather
  ...
  the mechanism, which is theirs (see `Descent.Condensation` for the ledger).
...
* **Symmetry-free.** The lattice arithmetic (`one_lt_latticeInflation`,
  ...
  of `Descent.Condensation`. These are the parts that apply to genotypes at
  every allele frequency — and they are the parts
  `Descent.PolygenicSpectroscopy` actually instantiates.

`Descent.PolygenicSpectroscopy` instantiates the symmetry-free side only. The split is
...
drift/condensation machinery of `Descent.Condensation`.
-/
...
`Descent.PolygenicSpectroscopy` states its hard-call comparison in this vocabulary. -/
