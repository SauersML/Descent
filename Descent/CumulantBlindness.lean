...
paragraph used to send the reader to `Descent.Condensation` for the condensation
mechanism and to `Descent.JetBarrier` for the completeness statement. Both files
...
Unlike the sign-erasure results of `Descent.EpistaticChaos` and the completeness
results of `Descent.JetBarrier`, **nothing in this file assumes the coordinate law is
...
caveat. Together with the drift theory of `Descent.Condensation`, this file is the
...
whatever the i.i.d. law is — and by `Descent.Condensation` the two can nonetheless
...
Put together with `Descent.Condensation`:
...
`Descent.JetBarrier` computes exactly what *is* observable there.
-/
