...
`Descent.ObservationalCeiling` states the law: a probe that reports the same data on
...
regimes of `Descent.DriftRegime` are separated as objects, and a regime declaration is
...
inductive StructuralGuard
  ...
  fails to compile (`Descent.Conventions`). -/
  | overDetermination
