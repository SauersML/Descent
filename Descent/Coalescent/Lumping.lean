...
theorem blocks_of_covers {n : ℕ} {ξ η : ER n} (h : Blindness.Covers ξ η) : blocks η + 1 = blocks ξ := h.2

/-- **K-G (5.1) vanishes off the diagonal below.**  There is no transition from a state with
`k` blocks to one with anything but `k - 1`, so the lumped sum is zero unless `v = k - 1`.
This is Kingman's "the sum (5.1) is 0 unless `u = f(ξ) = v + 1`". -/
theorem no_cover_of_blocks_ne {n : ℕ} {ξ η : ER n} (h : Blindness.Covers ξ η) {v : ℕ}
...
theorem lumping_criterion {n : ℕ} (ξ ξ' : ER n) (h : blocks ξ = blocks ξ') :
    Nat.card {η : ER n // Blindness.Covers ξ η} = Nat.card {η : ER n // Blindness.Covers ξ' η} := by
...
theorem lumping_rate_eq_deathRate {n : ℕ} (ξ : ER n) :
    (Nat.card {η : ER n // Blindness.Covers ξ η} : ℝ) = deathRate (blocks ξ) :=
