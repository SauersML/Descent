...
/-- **The drift horizon of a Dirichlet gap**, isolated so it can be computed. Beyond this
    ...
    multiplicative noise, 4000 replicates, the observable the crossing time with the last step
    INTERPOLATED. Gap and `C` are swept independently over a 88%
...
Found by the vacuity mode of `validation/mutation`, which derived `False` from the
hypotheses. -/
...
**Estimating `λ` inherits its whole sampling error.** From 50 observations of an AR(1) with `λ = 1`,
`τ_c` has a 95% range of `[0.18, 2.29]` — a factor of `12.7`; even 1000 observations
give `[0.51, 0.92]`.
