...
def chiSquareBudget (P : Foundations.ExpFunctional Ω) (densityRatio : Ω → ℝ) : ℝ :=
...
theorem chiSquareBudget_at_no_shift (P : Foundations.ExpFunctional Ω) :
...
def directionalResidualCurvature (P : Foundations.ExpFunctional Ω)
    (X : Ω → ι → ℝ) (residual : Ω → ℝ) (direction : ι → ℝ) : ℝ :=
  P (fun ω ↦ (Foundations.dot direction (X ω) * residual ω) ^ 2)

/-- Moment movement generated by reweighting a residual with a density ratio. -/
def weightedResidualMoment (P : Foundations.ExpFunctional Ω)
...
theorem dot_weightedResidualMoment
    ...
    Foundations.dot direction (weightedResidualMoment P densityRatio X residual) =
      P (fun ω ↦
        (densityRatio ω - 1) * (Foundations.dot direction (X ω) * residual ω)) := by
  unfold weightedResidualMoment rawCrossMoment Foundations.dot Descent.Core.innerSum
  have hexpand :
      (fun ω ↦
        (densityRatio ω - 1) *
...
theorem weighted_residual_moment_directional_bound
    ...
    (X : Ω → ι → ℝ) (residual : Ω → ℝ) (direction : ι → ℝ)
 :
    Foundations.dot direction (weightedResidualMoment P densityRatio X residual) ^ 2 ≤
...
theorem projection_artifact_energy_le_chiSquare_mul_curvature
  ...
  have hpairing :
      Foundations.dot artifact (weightedResidualMoment P densityRatio X residual) =
...
theorem projection_shift_eq_genuine_add_artifact
    ...
    (hgenuine : (Foundations.secondMomentMatrix Q X).mulVec genuine =
      rawCrossMoment Q X (fun ω ↦ hNew ω - hOld ω))
    (hartifact : (Foundations.secondMomentMatrix Q X).mulVec artifact =
      residualScoreMoment Q X hOld u)
    (hinjective : Function.Injective (Foundations.secondMomentMatrix Q X).mulVec) :
    (fun i ↦ v i - u i) = fun i ↦ genuine i + artifact i := by
  apply hinjective
  rw [projection_shift_genuine_artifact_decomposition Q X hOld hNew u v htarget]
  rw [Foundations.matrix_mulVec_add, hgenuine, hartifact]
...
theorem coefficientEnergy_add
    ...
    coefficientEnergy B (fun i ↦ x i + y i) =
      coefficientEnergy B x + coefficientEnergy B y +
        2 * Foundations.dot x (B.mulVec y) := by
  unfold coefficientEnergy
  rw [Foundations.matrix_mulVec_add, Foundations.dot_add_left]
  rw [Foundations.dot_add_right, Foundations.dot_add_right]
  rw [hsymmetric y x]
  ring
...
theorem total_shift_energy_le_sum_budget_sq
    ...
    (hcross : Foundations.dot genuine (B.mulVec artifact) ≤
...
* **Detection** of a weak spike.  The optimal linear certificate weights each
  eigendirection by the inverse background variance, `1 / σᵢ`: a direction with little background
  noise is a direction in which a small signal is visible.
...
where the *inverse* LD kernel is large.  `Descent.Blindness.ImitationRigidity` computes
...
`Descent.ImitationRigidity.ldWhiteningGain`.
...
/-- **Exact two-coordinate frontier.**  For a two-direction background with variances `σ₁, σ₂` and a
rank-one budget split as `(m, 1 - m)`, the
...
lemmas of `Descent.Spectral.QuadraticShift` apply on the nose. -/
...
theorem coefficientEnergy_rescaled_expand
    ...
    coefficientEnergy B (fun i ↦ a * beta i - theta i) =
      a ^ 2 * coefficientEnergy B beta -
        2 * a * Foundations.dot beta (B.mulVec theta) + coefficientEnergy B theta := by
...
def sharedCorrectionOptimum (B : Matrix ι ι ℝ) (beta theta : ι → ℝ) : ℝ :=
  Foundations.dot beta (B.mulVec theta) / coefficientEnergy B beta
...
def irreducibleDegradation (B : Matrix ι ι ℝ) (beta theta : ι → ℝ) : ℝ :=
  coefficientEnergy B theta -
    Foundations.dot beta (B.mulVec theta) ^ 2 / coefficientEnergy B beta
...
theorem irreducibleDegradation_le_rescaled
  ...
  have hkey : 2 * a * Foundations.dot beta (B.mulVec theta) -
      a ^ 2 * coefficientEnergy B beta ≤
      Foundations.dot beta (B.mulVec theta) ^ 2 / coefficientEnergy B beta := by
    rw [le_div_iff₀ hbeta]
    nlinarith [sq_nonneg (a * coefficientEnergy B beta -
      Foundations.dot beta (B.mulVec theta))]
...
theorem rescaled_at_optimum_eq_irreducibleDegradation
  ...
  linear_combination
    (Foundations.dot beta (B.mulVec theta) ^ 2 * (coefficientEnergy B beta)⁻¹) * hu
...
theorem rescaled_energy_sub_irreducible_eq_sq
  ...
  have hu : coefficientEnergy B beta * (coefficientEnergy B beta)⁻¹ = 1 :=
    mul_inv_cancel₀ (ne_of_gt hbeta)
  linear_combination
    (2 * a * Foundations.dot beta (B.mulVec theta) -
      Foundations.dot beta (B.mulVec theta) ^ 2 * (coefficientEnergy B beta)⁻¹) * hu
