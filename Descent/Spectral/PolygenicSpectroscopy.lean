...
at the size-bias point, and by `Descent.Blindness.Condensation` it is exactly the quantity
...
`Descent.Portability.ScoreDistribution` applies at `m = 1`. Define
...
`validation/empirical/condensation/check_condensation.py`, which recomputes all of this
...
*first* observable is blind: `Descent.Blindness.Condensation`'s drift-separation theorem has
...
*do* see it is the trichotomy conjecture of `Descent.Blindness.JetBarrier`, which that file
...
at which the three values of `log x ^ 2` form an **exact arithmetic progression** with span `h = log
((1 - q*) / q*) = log (3 + 2 sqrt 2) = 1.7627...`, so the hard-call law
...
`Descent.Portability.ImputationPortability`, which is an additive second-moment effect fully
...
`Descent.Blindness.HiddenConeAmbiguity` shows that the decay profile of latent loadings is
...
`Descent.Foundations.Probability`, which this module imports transitively.  A second copy
...
mean of `log x ^ 2` under the size-biased law. By `Descent.Blindness.Condensation` this is
...
`validation/empirical/condensation/check_condensation.py`, which evaluates this sum
numerically and checks it against `hweMellinDrift` across the frequency
spectrum. -/
noncomputable def mellinDrift (h : Foundations.HardyWeinbergModel) : ℝ :=
...
`validation/empirical/condensation/check_condensation.py`. No free parameter. -/
...
variable {h : Foundations.HardyWeinbergModel}

private theorem hwe_variance_eq (h : Foundations.HardyWeinbergModel) :
...
theorem _root_.Descent.Foundations.HardyWeinbergModel.mellinDrift_eq (h :
  ...
  have hC : q ^ 2 * (2 * (1 - q) / q) = 2 * q * (1 - q) := by
    field_simp
  unfold Foundations.HardyWeinbergModel.mellinDrift hweMellinDrift
  rw [Foundations.sum_over_genotypes, hX0, hX1, hX2, hP0, hP1, hP2, hA, hB, hC]
...
`Descent.Blindness.Condensation` already separates a balanced hard-called locus from its
...
`Descent.Condensation.log_two_lt_condensationConstant`. It was for a long time stated
...
direction, VALIDATED on the rare-variant tail (`validation/empirical/safe_order/`). The
...
`Descent.Blindness.JetBarrier` conjectures to be observable. A hard-called genotype has
...
`Descent.JetBarrier.inflated_intensity_ne_of_injective`, whose local-limit and
...
theorem _root_.Descent.Foundations.HardyWeinbergModel.mellinJetVariance_eq (h :
  ...
  unfold Foundations.HardyWeinbergModel.mellinJetVariance hweMellinJetVariance
  rw [Foundations.sum_over_genotypes, hX0, hX1, hX2, hP0, hP1, hP2, hA, hB, hC, hdrift]
...
`Descent.EpistaticChaos.centeredDosageSquare_two_valued_at_half` records from the
...
`Descent.EpistaticChaos.standardizedGenotype_symmetric_iff` proves that a
standardized Hardy-Weinberg genotype is sign-symmetric exactly at `q = 1/2`. So the
one frequency where the symmetric-law results of `Descent.Blindness.JetBarrier` may be
...
condensation-window theory of `Descent.Blindness.Condensation` — which needs a non-degenerate
...
`Descent.Blindness.CramerStratum` proves that a lattice coordinate law violates Cramér's
...
Note what the `2` is and is not. It is **not** the ploidy convention `Descent.ploidy`
...
`Descent.mellinDrift_uses_ploidy`. So the chain from this `2` down to `ploidy` is
...
theorem hardCall_logSquare_lattice_at_critical_maf (h : Foundations.HardyWeinbergModel)
    (hq : h.altFreq = latticeCriticalMaf) (u v : Foundations.DiploidGenotype) :
...
The load-bearing consequence, and the reason `Descent.Blindness.CramerStratum` is imported by
...
`Descent.EpistaticChaos.standardizedGenotype_symmetric_iff` were three quantities that
...
`q = 1/2` (`Descent.EpistaticChaos.standardizedGenotype_symmetric_iff`), while
...
symmetric (`Descent.EpistaticChaos.centeredSquare_rademacher_at_half`,
...
datum, which by `Descent.EpistaticChaos.standardizedGenotype_symmetric_iff` holds for a
...
`Descent`, so no text may cite it as proved here. It is cited only to say
...
theorem _root_.Descent.Foundations.HardyWeinbergModel.standardizedFourthMoment_eq (h :
  ...
  obtain ⟨hX0, hX1, hX2⟩ := standardizedSquare_values h hq0 hq1
  obtain ⟨hP0, hP1, hP2⟩ := genotypeProb_values h
  unfold Foundations.HardyWeinbergModel.standardizedFourthMoment
  rw [Foundations.sum_over_genotypes, hX0, hX1, hX2, hP0, hP1, hP2]
  field_simp
  ring
...
theorem standardizedFourthMoment_ge_two (h : Foundations.HardyWeinbergModel)
    (hq0 : 0 < h.altFreq) (hq1 : h.altFreq < 1) :
...
symmetry (`Descent.EpistaticChaos.standardizedGenotype_symmetric_iff`). The two are not
a coincidence: both descend from `Binomial(2, q)` being a symmetric law exactly at
`q = 1/2`, so the right framing is common cause. -/
theorem standardizedFourthMoment_eq_two_iff_half (h : Foundations.HardyWeinbergModel)
    (hq0 : 0 < h.altFreq) (hq1 : h.altFreq < 1) :
...
Combined with `Descent.EpistaticChaos.standardizedGenotype_symmetric_iff` — symmetry
...
theorem standardizedFourthMoment_ne_gaussian_at_half (h : Foundations.HardyWeinbergModel)
...
* `Descent.Portability.ScoreDistribution` and the Berry-Esseen certificates of
  `Descent.Foundations.Probability` are **untouched**: they concern degree-one aggregates,
...
* `Descent.PopGen.EpistasisAndNonAdditivity` is where the boundary bites. Its
...
* `Descent.Portability.ImputationPortability` gains a second, non-additive mechanism: beyond
...
* `Descent.Portability.PCCorrectability` is unaffected in content and sharpened in status: it
  answers "what does correction achieve given a convention", and
  `Descent.Blindness.HiddenConeAmbiguity` proves the convention cannot be replaced by an
  inference.
* Summary-statistic coherence checks are bounded-radius audits, and
  `Descent.Conditionals.LocalToGlobalCoherence` proves no such audit certifies joint
...
and the direct formulas in `Descent.Blindness.Condensation` require no symmetry field. That is
