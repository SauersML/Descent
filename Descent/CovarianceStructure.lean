...
This is `genotypeVarianceHWE` from `Descent.AncestrySpecificPower`. Do not define it
...
/-- **LDSR regression model: per-SNP expected squared marginal effect.**
    ...
    (`validation/empirical/simcov/battery_ldsc.py`, `test_ldsc`).
...
This is `admixedAlleleFreq` from `Descent.DemographicHistory` -- one function of a
...
/-- **Admixture LD magnitude.**
    ...
    Empirical status: **VALIDATED** (`validation/empirical/popgensel/admixcell.py`,
