...
survives a generation, where an additive body would leave nothing. -/
theorem ldRetentionPerGen_reference :
    ldRetentionPerGen (1 / 2) 1 = 1 / 4 := by
  unfold ldRetentionPerGen
  norm_num
...
disequilibrium with anything and `r²` is undefined. The divisor is zero and Lean returns `0`,
which reads as a tag that is present and informative-free rather than as a tag that is not
segregating -- so a monomorphic marker enters a pruning or scoring pipeline as an ordinary
...
/-- **Admixture linkage disequilibrium peaks at equal ancestry, pinned.** This definition carries
no result of its own. The ancestry factor is `α (1 - α)`, which is a quarter at equal ancestry --
the maximum -- rather than one half; the disequilibrium generated by admixture is quadratic in
the mixing fraction, not linear. -/
theorem admixtureLD_equal_ancestry :
    admixtureLD (1 / 2) 1 1 = 1 / 4 := by
  unfold admixtureLD
  norm_num
...
/-- **One generation of the Sved (1971) drift--recombination recurrence** for
    ...
    **Pending refactor.** The owner of `PortabilityDrift.lean` is extracting the
    ...
    (`validation/empirical/simcov/battery_core2.py`,
...
/-- **Per-generation retention factor of the two-locus identity measure**,
    ...
    Empirical status: **VALIDATED**
    (`validation/empirical/simcov/battery_dis1.py`). Two-locus
...
/-- **The trajectory of the two-locus identity measure** from an initial level
    ...
    (`validation/empirical/simcov/battery_traj.py`,
...
/-- **Per-generation drift rate at effective size Ne**, `1/(2Ne)`. Larger Ne means
    ...
    **FALSIFIED** (`validation/empirical/coalescent_diff/`).
...
instantaneous and complete, so the per-generation rate diverges; the divisor is zero and Lean
returns `0`, reporting a population that never drifts. This is the reverse of the truth and it
propagates: every retention and decay quantity built on this rate inherits it. Consumers must
require `Ne ≠ 0`. -/
...
/-- **The linkage-disequilibrium half-life at zero effective size, named, and compounded.** Two
junk branches fire in sequence. At `Ne = 0` the drift term `1 / (2 Ne)` is junk-zero, so retention
is one -- a population that never loses disequilibrium. The logarithm of one is zero, so the
half-life divides by zero and is junk-zero in turn: an instantaneous half-life reported for
disequilibrium that in fact decays never. Neither branch is visible in the value, and a chain of
two junk values cannot be diagnosed from the output alone. Consumers must require `Ne ≠ 0` and
retention strictly below one. -/
theorem ldHalfLife_zero_population_is_junk :
    ldHalfLife 0 0 = 0 := by
  unfold ldHalfLife ldRetentionPerGen
  norm_num
...
/-- **Retention compounds geometrically, pinned.** This definition carries no result of its own.
Two generations at quarter retention leave a sixteenth, not an eighth: the per-generation factor
is raised to the number of generations rather than multiplied by it. -/
theorem ldRetainedFraction_two_generations :
    ldRetainedFraction (1 / 2) 1 2 = 1 / 16 := by
...
/-- **LD recurrence relation.**
    ...
    Empirical status: **VALIDATED**
    (`validation/empirical/simcov/battery_bulk14.py`). Two-locus
