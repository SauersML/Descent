...
import Descent.PCCorrectability.Threshold
...
`Descent.ImitationRigidity` proves the algebraic half of the imitation
...
`Descent.ImitationRigidity` proves that a rank-one bump with unequal squared
...
`Descent.ImitationRigidity` computes, for a stationary first-order LD kernel
...
`Descent.PCCorrectability.Threshold` already contains a detection threshold
...
* `demographicSpike n F m` is the *certificate increment* — the spike level
  ...
  intentionally convention-agnostic: `Descent.DemographicCapacity`
...
`validation/empirical/meff_prohibition/check_meff_prohibition.py` builds both
...
Everything about basis dependence of the free entropy -- the isospectral block
pair, the entrywise cube and fourth power invariants, the cumulant contractions
that produce them, the order-two coincidence, and the LD power-score form -- is in
`Descent/SpectralUniversalityFailure.lean`.

A parallel version was developed here and is gone. It was drafted while that file
was being written, and the two overlapped completely by the end: the general
statements there carry the cumulant scalar, use Mathlib's `Matrix` API over
arbitrary index types, and include the order-two coincidence
(`entrySquareSum_eq_trace_sq_of_symmetric`) and the reference-panel form
(`ldOrientationFourthInvariant_eq_sum_ldPowerScore_four`) that the version here
also reached. Keeping both would have been two statements of one theorem in two
modules, which is the duplication this corpus has already paid for once.
-/
