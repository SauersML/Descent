...
noncomputable instance finite : Finite (FinitePartialBijection Y Z) := by
  ...
  have hg : (encode b).2.2 = (encode c).2.2 := congrArg (fun p ↦ p.2.2) hbc
  have hbmem : (y, b.apply y hy) ∈
      (encode b).2.2 := by
    simp [encode, hy]
  -- REPAIR ON EXTRACTION: `hg ▸ hbmem` elaborated under Lean 4.32 but not under the
  -- 4.24 toolchain this repository pins, which cannot infer the motive here.
  -- Rewriting explicitly says the same thing and is version-independent.
  have hcmem : (y, b.apply y hy) ∈ (encode c).2.2 := by rw [← hg]; exact hbmem
...
theorem exists_extension (b : FinitePartialBijection Y Y) :
  ...
  obtain ⟨p, hp⟩ := perm_exists_extending_pair inclusion target hi ht
