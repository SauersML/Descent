...
`Core.Fst.hweHeterozygosity q = ploidy · q · (1 - q)` was written as a scalar formula with a
biological name and no genotype anywhere in its type.  This is the statement that the
