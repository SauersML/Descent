...
It is gone: `Descent/LandscapeSuperposition.lean` has the same material
and more.
...
The theorem below is stated about `f1Score` from `Descent.OpenQuestions`, which this
...
`Descent.FoldedSpectrum` sharpens that claim, and in doing so narrows it.
...
* they agree in sensitivity and in specificity -- and this is the part that needs
  `Descent.FoldedSpectrum`, since it holds for *every* level-set functional at once, not
...
`Descent.ProjectionShiftBounds` proves the general statement: among relaxed
...
the first-order Markov law of `Descent.ImitationRigidity` — correlation `ρ^d`
...
-- `HasTaskIndependentSpectralPortabilityScalar` is defined in `Descent.SpectralDegradation`,
