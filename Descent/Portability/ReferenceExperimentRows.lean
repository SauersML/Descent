...
tabled terminal law is the context-mass average of its corner accumulators
(`ReferenceExperimentCorners.tableReport_eq_corners`), each corner accumulator is decided in its
own small lemma in one of the four corner certificate modules, and the row is the norm_num
combination of the four corners with the context masses. The corner values are recorded in
`validation/reference_experiment_corners.json`, where their context-mass averages are checked
