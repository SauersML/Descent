...
/-- **The composed target `R²` for a clean two-branch split, with the branches separated.**
    ...
    also closes the `NeT` question that `cleanSplitTargetR2'_NeT_enters_only_through_chart`
    states — that theorem and the measurement are about the same thing, and each is where
    the reader should look for the other. 19 cells, worst 1.97 sems and 0.33% relative, residuals
...
That historical failure forced the bracket below. The UPPER end applies the
drift-and-migration penalty and no LD
    ...
    r2_0 * moments.panelTransportRatio panel hetRho source target domain =
      r2_0 *
        ((((∑ k, moments.DD (panel k) source target) /
          ...
          ((moments.H hetRho source source / moments.H hetRho target target) ^ 2)) := by
  rfl
