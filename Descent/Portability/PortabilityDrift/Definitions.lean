...
theorem fstFromTau_negative_unit_tau_is_junk :
    Descent.Core.fstFromTau (Descent.Core.Tau.ofScaled (-1)) = 0 := by
...
including `s = 0`. -/
theorem one_div_transferTimeInflation_eq_one_sub_saturation (s a : ℝ) :
    1 / Spectral.transferTimeInflation s a = 1 - Descent.Core.saturation ((a / s) ^ 2) := by
...
theorem pairwiseFstFromBranches_eq_fstFromTau_add_mul (a b : ℝ)
    (ha : 0 ≤ a) (hb : 0 ≤ b) :
    pairwiseFstFromBranches (Descent.Core.fstFromTau (Descent.Core.Tau.ofScaled a))
        (Descent.Core.fstFromTau (Descent.Core.Tau.ofScaled b)) =
      Descent.Core.fstFromTau (Descent.Core.Tau.ofScaled (a + b + a * b)) := by
  ...
  unfold pairwiseFstFromBranches Descent.Core.fstFromTau Descent.Core.saturation
    Descent.Core.complementaryComposition
  simp only [Descent.Core.Tau.value_ofScaled]
  field_simp
  ring
...
theorem pairwiseFstFromBranchTaus_lt_pairwiseFstFromBranches (a b : ℝ)
    ...
    pairwiseFstFromBranchTaus a b <
      pairwiseFstFromBranches (Descent.Core.fstFromTau (Descent.Core.Tau.ofScaled a))
        (Descent.Core.fstFromTau (Descent.Core.Tau.ofScaled b)) := by
...
theorem pairwiseFst_composition_gap_eq (a : ℝ) (ha : 0 ≤ a) :
    pairwiseFstFromBranches (Descent.Core.fstFromTau (Descent.Core.Tau.ofScaled a))
          (Descent.Core.fstFromTau (Descent.Core.Tau.ofScaled a)) -
...
theorem fstFromTau_lt_coalescenceCdf (tau : ℝ) (htau : 0 < tau) :
    Descent.Core.fstFromTau (Descent.Core.Tau.ofScaled tau) < 1 - Real.exp (-tau) := by
...
theorem fstFromTau_at_one_time_unit :
    Descent.Core.fstFromTau (Descent.Core.Tau.ofScaled 1) = 1 / 2 := by
  unfold Descent.Core.fstFromTau Descent.Core.saturation
  simp only [Descent.Core.Tau.value_ofScaled]
  norm_num
...
/-! ### `genotypeVarianceHWE`, moved here so two modules that cannot see each other can name it

`PortabilityDrift/PresentDayMoments` states the allele-frequency retention as a ratio of
these, and `AncestrySpecificPower` -- where this used to live -- imports the
`PortabilityDrift` head, so the definition could not stay there without a cycle. It is the
same declaration in the same namespace, so every existing caller resolves unchanged:
anything that reached `AncestrySpecificPower` reaches this file through that same head
import. -/
