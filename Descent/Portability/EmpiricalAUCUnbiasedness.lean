...
/-!
# Exact finite-cohort AUC, calibration slope and Brier laws

NOTE1 (42) records four exact finite-cohort statements for an independent cohort of size `n`
drawn from a four-cell law on score and outcome: the empirical AUC is defined with probability
`1 − P_Y(1)ⁿ − P_Y(0)ⁿ`; conditional on definedness its expectation is the population AUC;
conditional on the score varying, the empirical least-squares slope has expectation the
population slope; and the empirical Brier score is unbiased for the population one.

This module builds the empirical statistics on a cohort sample and proves all four, first for
an arbitrary four-cell law and then at the chronology cells of NOTE1 (31), where they read
`1 − pⁿ − (1 − p)ⁿ`, `(1 + C) / 2`, `C` and `2 h (1 − C)`. The empirical AUC is the ordered
case-control average of the corpus `empiricalAUCComparison`, the same half-credit ranking rule
the corpus population AUC uses, so
...
finite product law, the Brier identity is the mean of a sum of identically distributed
coordinates, and the AUC and slope identities are rearrangements of finite sums over the cohort
sample space. No measurement on any cohort can bear on them.
-/
...
theorem sum_prod_pair {n : ℕ} (first second : Fin n) (hne : first ≠ second)
  ...
  have hone := hterm true true
  have htwo := hterm true false
  have hthree := hterm false true
  have hfour := hterm false false
  simp only [Fintype.sum_bool] at hone htwo hthree hfour
  rw [hswap]
  simp only [Fintype.sum_bool]
  linear_combination hone + htwo + hthree + hfour
...
theorem outcome_fiber_numerator (law : FiniteReportLaw (Bool × Bool)) {n : ℕ}
  ...
  have houter : ∀ caseMember : Fin n,
    ...
    rw [Finset.mul_sum]
    exact Finset.sum_congr rfl fun controlMember _ ↦ hterm caseMember controlMember
  have hfactorInner : ∀ caseMember : Fin n,
      (∑ controlMember : Fin n,
        (if outcomes caseMember && !outcomes controlMember then (1 : ℝ) else 0) *
          (law.binaryAUCNumerator scoreOf caseOf *
  ...
  have hfactor : (∑ caseMember : Fin n, ∑ controlMember : Fin n,
      ...
      (∑ caseMember : Fin n, ∑ controlMember : Fin n,
        ...
        (law.binaryAUCNumerator scoreOf caseOf *
          ∏ member, outcomeMass law (outcomes member)) := by
    rw [Finset.sum_mul]
    exact Finset.sum_congr rfl fun caseMember _ ↦ hfactorInner caseMember
