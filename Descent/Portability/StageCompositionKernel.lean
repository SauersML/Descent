...
(`apply_reindex`), and `composeStages` runs a finite list of stages `Fin n → kernel` in order,
with branch type `Fin n → B` (`apply_composeStages_zero`, `apply_composeStages_succ`).
...
`compose_expansion` is the two-stage law and `composeStages_expansion` the law for any finite
number of stages, both with an explicit remainder (`compositeSlack`): the per-stage slacks plus
`h` times a constant built from a uniform feature bound, a uniform row bound on the stage
matrices, and the stage count.  `composedMicroscopicApproximation` packages this as the
...
def idleKernel {B : Type*} [Fintype B] : FiniteMixtureKernel (Fin 0 → B) X where
  weight _ _ := 1
  move _ x := x
...
theorem apply_idleKernel {B : Type*} [Fintype B] (observable : X → ℝ) (x : X) :
    (idleKernel (B := B) (X := X)).apply observable x = observable x := by
  simp [FiniteMixtureKernel.apply, idleKernel]

/-- The step made of `n` stages run in order: the first stage, then the step made of the
remaining stages.  A branch records the branch drawn at every stage. -/
def composeStages {B : Type*} [Fintype B] :
    (n : ℕ) → (Fin n → FiniteMixtureKernel B X) → FiniteMixtureKernel (Fin n → B) X
  | 0, _ => idleKernel
  | n + 1, stages =>
      reindex (Fin.consEquiv fun _ : Fin (n + 1) ↦ B : B × (Fin n → B) ≃ (Fin (n + 1) → B))
        (compose (stages 0) (composeStages n (Fin.tail stages)))
...
theorem apply_composeStages_succ {B : Type*} [Fintype B] (n : ℕ)
    ...
    (composeStages (n + 1) stages).apply observable x =
      (stages 0).apply ((composeStages n (Fin.tail stages)).apply observable) x := by
  simp only [composeStages]
...
/-- **NOTE1 equation (11) for a composed step.**  If every one of finitely many stages advances
each feature by `step` times its own matrix applied to the features, up to `step` times its
slack, then running the stages in order advances each feature by `step` times the sum of the
stage matrices, up to `step · compositeSlack`: the total slack plus a cross term of order
`step`. -/
theorem composeStages_expansion {ι B : Type*} [Fintype ι] [Fintype B]
  ...
  | zero =>
    intro stages generator slack _ _ _ x i
    rw [apply_composeStages_zero]
    simp [compositeSlack]
  | succ n ih =>
    intro stages generator slack hrow hslack hstages x i
    have htail := ih (Fin.tail stages) (fun k ↦ generator k.succ) (fun k ↦ slack k.succ)
      (fun k j ↦ hrow k.succ j) (fun k ↦ hslack k.succ) (fun k y j ↦ hstages k.succ y j)
    ...
    have hbinary := compose_expansion feature bound hboundNonneg hbound (stages 0)
      (composeStages n (Fin.tail stages)) (generator 0) (∑ k : Fin n, generator k.succ)
      rowBound ((n : ℝ) * rowBound) (hrow 0) htailRow step (slack 0)
      (compositeSlack n rowBound bound step (∑ k : Fin n, slack k.succ)) hstep hrowNonneg
      (hslack 0) (hstages 0) htail x i
    rw [apply_compose] at hbinary
    rw [apply_composeStages_succ, Fin.sum_univ_succ, Fin.sum_univ_succ]
    ...
    have hidentity : compositeSlack (n + 1) rowBound bound step
      ...
      ring
    rw [hidentity]
    calc |(stages 0).apply ((composeStages n (Fin.tail stages)).apply fun y ↦ feature y i) x -
...
def composedMicroscopicApproximation {ι B : Type*} [Fintype ι] [Fintype B]
  ...
  error_tendsto := by
    ...
    have hcomposite : Filter.Tendsto
        (fun step ↦ compositeSlack stageCount rowBound bound step (∑ k, slack k step))
        (nhdsWithin 0 (Set.Ioi 0)) (nhds 0) := by
      have hlimit := hsum.add (hidentityLimit.mul (hconstant.mul (hconstantBound.add hsum)))
