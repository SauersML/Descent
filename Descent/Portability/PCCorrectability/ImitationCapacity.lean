...
`Descent.Blindness.ImitationRigidity` proves the algebraic half of the imitation
...
`Descent.Blindness.ImitationRigidity` proves that a rank-one bump with unequal squared
...
`Descent.Blindness.ImitationRigidity` computes, for a stationary first-order LD kernel
...
* `demographicSpike n F m` is the *certificate increment* — the spike level
  ...
  intentionally convention-agnostic: `Descent.PopGen.DemographicCapacity`
...
`Descent/Blindness/SpectralUniversalityFailure.lean`.
