...
/-- **PGS variance under HWE and linkage equilibrium.**
    ...
    LD gap measured (`validation/empirical/simcov/battery_linalg.py`,
...
through `Descent.Probability.berryEsseenErrorBound` and so cannot drift from it.
...
replaced.** Measurements in `validation/empirical/block_count/`; positive control (`ℓ = 1`,
...
`Descent.Foundations.Probability` holds the one body for it. A second spelling in this file, such
...
`Descent.Blindness.Condensation` fixes the price of leaving that regime. A score aggregating terms
...
where it does not shrink. `Descent.Blindness.JetBarrier` supplies the missing factor.
