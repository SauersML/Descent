...
CDF was unavailable.  `Phi` is defined in `Descent.Foundations.Probability`, which this
...
/-- **Fraction of heritability attained at sample size `n`**: `n / (n + C)` with `C = M/h²`
    ...
    by `h²` — which is what `Descent.expectedR2FromN` does. The name says fraction because
    ...
    (`validation/empirical/simcov/battery_bulk8.py`,
