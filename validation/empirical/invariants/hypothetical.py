...
Run:  python3 validation/empirical/invariants/hypothetical.py
      python3 validation/empirical/invariants/hypothetical.py --json
      python3 validation/empirical/invariants/hypothetical.py --min-score 3
"""
...
# Three levels up, not two: this file sits at validation/empirical/
...
CORPUS = os.path.normpath(os.path.join(HERE, "..", "..", "..", "Descent"))
...
def main() -> int:
    ...
    if not os.path.isdir(CORPUS):
        print(f"cannot find {CORPUS}", file=sys.stderr)
        return 2

    findings = [f for f in analyze(load(CORPUS)) if f["score"] >= args.min_score]
