"""Parse Lean 4 declarations out of Descent into a structured table.
...
from paths import CORPUS, require, ARTIFACTS as ART  # noqa: E402


def _lean_sources(_r):
    # `rglob` under Descent/ does NOT reach `Descent.lean`, the corpus
...
class Decl:
    ...
    name: str
    module: str  # e.g. Descent.PortabilityDrift
    file: str
    line: int
    docstring: str = ""
...
def parse_file(path: Path) -> list[Decl]:
    ...
    lines = text.split("\n")
    # The corpus ROOT `Descent.lean` is a SIBLING of Descent/, so
    # `relative_to(CORPUS)` raises on it. Its module name is just `Descent`.
    if path.resolve() == (CORPUS.parent / "Descent.lean").resolve():
        module = "Descent"
    else:
        module = "Descent." + str(path.relative_to(CORPUS)).replace(".lean", "").replace("/", ".")
    decls: list[Decl] = []
...
def parse_all(root: Path = CORPUS) -> list[Decl]:
    out = []
    for p in _lean_sources(root):
        if ".lake" in p.parts:
            continue
        out.extend(parse_file(p))
    top = root.parent / "Descent.lean"
    if top.exists():
