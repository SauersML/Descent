...
HERE = Path(__file__).resolve().parent    # .../validation/empirical/symbolic
EMPIRICAL = HERE.parent                   # .../validation/empirical
VALIDATION = EMPIRICAL.parent             # .../validation
REPO = VALIDATION.parent                  # repo root -- the whole repo is the proofs
CORPUS = REPO / "Descent"
...
#   1. $DESCENT_ARTIFACTS          explicit override
...
def _resolve_artifacts() -> Path:
    import os
    override = os.environ.get("DESCENT_ARTIFACTS")
    if override:
        p = Path(override)
