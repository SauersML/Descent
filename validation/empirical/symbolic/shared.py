...
Definition bodies come from `validation/empirical/extract/api.py` and nowhere else.
...
EXTRACT = str(require(_EXTRACT, "validation/extract"))
...
def def_records():
    ...
    out = []
    for fq, d in definitions().items():
        module = "Descent." + d["file"].replace("Descent/", "").replace(".lean", "").replace("/", ".")
