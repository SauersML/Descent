"""BLIND GATE 4 (v3.2 on serial1d), stage 3: the single-shot grader.  Run ONCE, after the fresh seeds
...
def main():
    ...
    for pheno in ("phenoC", "phenoA"):
        ...
        for path in sorted(glob.glob(SEED_GLOB.format(pheno=pheno))):
            seed = path.split("g4s")[-1].split(".")[0]
            side = json.load(open(f"{TO}/gate4_l3/serial1d_realpt_g4s{seed}.json"))
            t = side["train_deme"]
    ...
    json.dump(verdict, open(f"{TO}/gate4_verdict.json", "w"), indent=1, sort_keys=True)
