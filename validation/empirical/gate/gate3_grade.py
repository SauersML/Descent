"""BLIND GATE 3 (v3.2 integral), stage 3: the single-shot grader.  Run ONCE, after the fresh seeds
...
import gate_predictor_v32 as GP

TO = "/projects/standard/hsiehph/sauer354/theory-out"
SEED_GLOB = f"{TO}/gate3_l3/grid2d_{{pheno}}_realpt_g3s*.parquet"
...
def main():
    ...
    for pheno in ("phenoC", "phenoA"):
        ...
        for path in sorted(glob.glob(SEED_GLOB.format(pheno=pheno))):
            seed = path.split("g3s")[-1].split(".")[0]
            side = json.load(open(f"{TO}/gate3_l3/grid2d_realpt_g3s{seed}.json"))
            t = side["train_deme"]
    ...
    json.dump(verdict, open(f"{TO}/gate3_verdict.json", "w"), indent=1, sort_keys=True)
