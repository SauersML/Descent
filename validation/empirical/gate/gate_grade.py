"""BLIND GATE, stage 3: the single-shot grader.  Run ONCE, after the fresh seeds
...
import gate_predictor as GP

TO = "/projects/standard/hsiehph/sauer354/theory-out"
SEED_GLOB = f"{TO}/gate_l3/grid2d_{{pheno}}_realpt_gate*.parquet"
...
def main():
    tables = GP.load_tables(f"{TO}/gate_tables.json")
    verdict = {"bars": {}, "seeds": {}}
    for pheno in ("phenoC", "phenoA"):
        seed_ratio_resid, seed_auc_resid = [], []
        for path in sorted(glob.glob(SEED_GLOB.format(pheno=pheno))):
            seed = path.split("gate")[-1].split(".")[0]
            side = json.load(open(f"{TO}/gate_l3/grid2d_realpt_gate{seed}.json"))
            t = side["train_deme"]
    ...
    json.dump(verdict, open(f"{TO}/gate_verdict.json", "w"), indent=1, sort_keys=True)
