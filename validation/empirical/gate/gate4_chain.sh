...
# All outputs under theory-out/gate4_l3/ and theory-out/gate_{tables,verdict}.json.
set -u
...
mkdir -p "$TO/gate4_l3"
cd "$SIMS" || exit 1
for seed in 201 202 203 204 205 206 207 208; do
  setsid nohup $PY gen_stress_pt.py serial1d "$TO/gate4_l3" $seed --tag _g4s$seed \
    --threads 3 > "$TO/gate4_l3/run_s$seed.log" 2>&1 < /dev/null &
done
# wait for all 8 sidecars (grid2d_realpt_gate<seed>.json), up to 24h
for i in $(seq 1 1440); do
  n=$(ls "$TO"/gate4_l3/serial1d_realpt_g4s*.json 2>/dev/null | wc -l)
  [ "$n" -ge 8 ] && break
  sleep 60
done
$PY "$GATE/gate4_grade.py" > "$TO/gate4_verdict.log" 2>&1
