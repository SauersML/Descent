"""GATE PREDICTOR v3.2-serial1d -- the identical selection-conditioned winner-location
integral law, pointed at the serial1d founder-cascade demography. PURE; pinned for the
fourth blind gate (serial1d seeds 201-208, never generated at pin time). SAME LAW,
ZERO refitting: only the analytic tables change (serial1d_gate_tables.json: epoch-
composed DD/H on the 14-point rho grid via the E5-validated integrator; ascertained
degree-4 moments propagated through every founding split by the polynomial-in-p0
basis). The winner density uses the TRAIN deme's own LD profile, since serial1d demes
have different founding ages. Consistency check passed at table build: P_asc equals
the grid2d value exactly, as the shared ancestral law requires.
...
def load_all():
    t = json.load(open(f"{TO}/serial1d_gate_tables.json"))
    rhog = sorted(float(k) for k in t["byrho"])
    tabs = {}
    for r in rhog:
        blk = t["byrho"]["%g" % r]
        tabs[r] = {}
        for key, v in blk["DD"].items():
            i, j = key.split("_")
            tabs[r][(int(i), int(j))] = float(v)
    H0 = t["byrho"]["%g" % rhog[0]]["H"]
    return {"H": H0, "asc": t["asc"], "rhog": rhog, "tabs": tabs, "D": 10}
...
def _win_density(tables, z, t=0):
    rhog, tabs = tables["rhog"], tables["tabs"]
    r2p = [tabs[r][(t, t)] / tabs[rhog[0]][(t, t)] for r in rhog]
...
def predict(tables, train_deme, r2_train, p_threshold):
    ...
    xs, Pw = _win_density(tables, z, t=train_deme)
    rho_of = lambda x: 4 * NREF * C0 * abs(x)
