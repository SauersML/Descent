...
# All outputs under theory-out/gate3_l3/ and theory-out/gate_{tables,verdict}.json.
set -u
...
mkdir -p "$TO/gate3_l3"
cd "$SIMS" || exit 1
for seed in 117 118 119 120 121 122 123 124; do
  setsid nohup $PY gen_stress_pt.py grid2d "$TO/gate3_l3" $seed --tag _g3s$seed \
    --threads 3 > "$TO/gate3_l3/run_s$seed.log" 2>&1 < /dev/null &
done
# wait for all 8 sidecars (grid2d_realpt_gate<seed>.json), up to 24h
for i in $(seq 1 1440); do
  n=$(ls "$TO"/gate3_l3/grid2d_realpt_g3s*.json 2>/dev/null | wc -l)
  [ "$n" -ge 8 ] && break
  sleep 60
done
$PY "$GATE/gate3_grade.py" > "$TO/gate3_verdict.log" 2>&1
