"""Round 2 of the Descent definition checks.
...
def check_split_fst(args):
    ...
    ts = msprime.sim_ancestry(samples={"A": 30, "B": 30}, demography=dem,
                              sequence_length=float(os.environ.get("LEN", "2e7")), recombination_rate=RHO,
                              random_seed=seed)
...
def check_admixture(args):
    ...
    ts = msprime.sim_ancestry(samples={"A": 30, "B": 30, "ADM": 30},
                              demography=dem, sequence_length=float(os.environ.get("LEN", "2e7")),
...
def main():
    ...
    if which in ("all", "island"):
        for ndemes in [int(x) for x in os.environ.get("DEMES", "2,5,20,50").split(",")]:
