...
WHY THIS EXISTS.  Every results file under `validation/` that a Python
...
def _find_repo():
    ...
    This module was written at `validation/empirical/simprov.py` and used
    `parents[2]`. It was then moved one level deeper, to
    `validation/empirical/`, and `parents[2]` silently became
    `<repo>/proofs`.

    Nothing crashed. What broke instead was the dirty-tree exclusion below:
    a pathspec of `validation/**/*.json` evaluated from inside
    `proofs/` means `proofs/validation/...`, matches nothing, and every
