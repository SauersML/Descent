...
# REGENERATE WITH:  python3 validation/empirical/extract/emit.py
#
...
# the corpus on disk right now. They are generated from Descent/,
...
def build_context(blob):
    ...
    def qualified_resolver(dotted):
        ...
        """
        for cand in (dotted, f"Descent.{dotted}"):
            if cand in by_name:
                return pynames[cand]
        return None
    ...
    def dot_resolver(ty, meth, nargs):
        ...
        for cand in (f"Descent.{ty}.{meth}", f"{ty}.{meth}"):
            if cand in by_name:
                return pynames[cand]
