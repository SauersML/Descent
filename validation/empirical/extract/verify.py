...
def adversarial_vectors():
    ...
    fn, args = api.callable_for("Descent.harmonicMeanNe")
    ...
    fn, args = api.callable_for("Descent.ldMismatchFrobenius")
...
def no_swallowed_declarations():
    ...
    # NOT root-inclusive, deliberately. Adding `Descent.lean` here is
    # correct for a reference scan and WRONG here: this list feeds test_parser.py
    # and leanexpr.extract_file, which rebuild a path as `Descent/<rel>` and
    ...
    sources = sorted((PROOFS / "Descent").rglob("*.lean"))
    root = PROOFS / "Descent.lean"
    if root.exists():
...
def collision_handling_actually_works():
    ...
    rows = []
    for file, line in (("Descent/A.lean", 10), ("Descent/B.lean", 20)):
        f = Fake()
        f.name, f.file, f.line, f.signature = "Descent.dup", file, line, "(x : R) : R"
        rows.append(f)
    got = lean_parse.find_collisions(rows)
    assert "Descent.dup" in got, "find_collisions missed a real duplicate"
    assert len(got["Descent.dup"]) == 2, got
    print(f"  ok  duplicate detected: {got['Descent.dup']}")

    # and a table built over that blob must exclude it and raise on lookup
    import api
    real = api._blob()
    patched = dict(real)
    patched["collisions"] = {"Descent.dup": got["Descent.dup"]}
    patched["definitions"] = real["definitions"] + [
        {**real["definitions"][0], "name": "Descent.dup"}]
    ...
    try:
        ...
        assert "Descent.dup" not in api.definition_table(), \
            "a collided name leaked into the table"
        try:
            api.definition("Descent.dup")
...
def finite_index_types_translate_correctly():
    ...
    fn, argnames = api.callable_for("Descent.Pop.pair")
    ...
    fn, _a = api.callable_for("Descent.haplotypeHomozygosity")
    for n in (2, 4, 5):
    ...
    fn, argnames = api.callable_for("Descent.gramForm")
    A = [[1.0, 2.0], [3.0, 4.0]]
    ...
    assert api.vector_args("Descent.gramForm")["A"]["rank"] == 2, \
