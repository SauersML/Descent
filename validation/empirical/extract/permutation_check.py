...
def full_tree(rev):
    ...
    out = {}
    listing = git("ls-tree", "-r", "--name-only", rev, "Descent").split()
    for f in listing:
        if f.endswith(".lean"):
...
def scan(n_commits):
    ...
    log = git("log", "--format=%H", f"-{n_commits}", "--", "Descent").split()
    rng = random.Random(20260802)

    for rev in log:
        files = [f for f in git("show", "--name-only", "--format=", rev).split()
                 if f.endswith(".lean") and "Descent" in f]
        if not files:
            continue
        before, after = {}, {}
        for f in files:
...
def main(argv=None):
    ...
    oldest = git("log", "--format=%h %ad %s", "--date=short",
                 f"-{a.commits}", "--", "Descent").strip().splitlines()
    if oldest:
