"""Parser for the Descent Lean corpus.

Produces a machine-readable table of every declaration under Descent/,
...
Usage:
    python3 lean_parse.py [--root Descent] [--out defs.json]
"""
...
def find_proofs_root(start: pathlib.Path) -> pathlib.Path:
    """Walk up from `start` to the directory that actually holds `Descent/`.

    Every module under validation/ used to hardcode its own depth
    (`HERE.parent.parent`).  When this directory moved from
    `validation/extract` to `validation/empirical/extract` those
    constants all became one level short, so `root` pointed at
    `validation/Descent`, which does not exist.  `rglob` on a missing
    ...
    for cand in (start, *start.parents):
        if (cand / "Descent").is_dir():
            return cand
    raise CorpusNotFound(
        f"no ancestor of {start} contains a Descent/ directory; the Lean "
...
def main(argv=None):
    ...
    ap.add_argument("--root", default="Descent")
