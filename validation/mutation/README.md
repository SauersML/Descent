...
`validation/code/Check.lean` gates on hypotheses that occur nowhere in a
...
From the repository root, on a machine with a built corpus:
    ...
    bash validation/mutation/run.sh one    # one mutant per Prop binder
    bash validation/mutation/run.sh all    # one mutant per theorem, all binders dropped
